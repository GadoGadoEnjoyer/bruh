    ...
    public function __construct(){
        ...
        try{
            ...
            if($this->tableExist('Users')&&$this->tableExist('Votes')&&$this->tableExist('Users_Votes')&&$this->tableExist('Comments')){
                $this->db->query('ALTER TABLE Votes ADD FOREIGN KEY (creator_id) REFERENCES Users(id)');
                $this->db->execute();
                print_r("Foreign key creator_id added to Votes table\n");
                $this->db->query('ALTER TABLE Users_Votes ADD FOREIGN KEY (user_id) REFERENCES Users(id)');
                $this->db->execute();
                print_r("Foreign key user_id added to Users_Votes table\n");
                $this->db->query('ALTER TABLE Users_Votes ADD FOREIGN KEY (vote_id) REFERENCES Votes(id)');
                $this->db->execute();
                print_r("Foreign key vote_id added to Users_Votes table\n");
                $this->db->query('ALTER TABLE Comments ADD FOREIGN KEY (creator_id) REFERENCES Users(id)');
                $this->db->execute();
                print_r("Foreign key creator_id added to Comments table\n");
                $this->db->query('ALTER TABLE Comments ADD FOREIGN KEY (vote_id) REFERENCES Votes(id)');
                $this->db->execute();
